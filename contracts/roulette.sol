...
contract Roulette {
  ...
  uint betAmount = 100000000000000;
  uint necessaryBalance = 0;
  uint nextRoundTimestamp = block.timestamp;
  address payable creator = payable(msg.sender);
  uint256 maxAmountAllowedInTheBank = 2000000000000000000;
  ...
  */
  ...
  event RandomNumber1(uint256 number);
  ...
  function bet(uint8 number, uint8 betType) payable public {
    ...
    */
    require(msg.value == betAmount, "There is an error");                               // 1
    require(betType >= 0 && betType <= 5, "There is an error 2");                         // 2
    require(number >= 0 && number <= numberRange[betType], "There is an error 3");        // 3
    uint payoutForThisBet = payouts[betType] * msg.value;
    uint provisionalBalance = necessaryBalance + payoutForThisBet;
    require(provisionalBalance < address(this).balance, "There is an error 4");           // 4
  ...
  function spinWheel() public returns (uint){
    ...
    for (uint i = 0; i < bets.length; i++) {  //accumulate total over all bets
      bool won = false;
      Bet memory b = bets[i];
      if (number == 0) {
  ...
  function getWin() public view returns(uint){
    return winnings[owner];
  }

  function addvalue(uint256 number) public returns (uint){
      necessaryBalance += number;
      return(5);
      }
